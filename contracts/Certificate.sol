...
contract Certificate is ERC721URIStorage{
    ...
    function createToken(string memory tokenURI , address receiver) public payable returns (uint256)
    {
        _tokenIds.increment();
    ...
    function getOrganisationCertificates(address org) public view returns(ListedToken[] memory)
    {
    ...
    function getUserCertificates(address user) public view returns(ListedToken[] memory)
    {
        uint totalItemCount = _tokenIds.current();
        uint itemCount = 0;
        uint currentIndex = 0;
